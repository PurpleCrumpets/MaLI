function [vtk_output] = read_particle_vtk(filename)
...
% Combining Matrices
vtk_output = [ID points velocity force omega torque]; 

% Output as a Table
vtk_output = array2table(vtk_output);

% Merging Columns under one heading
vtk_output = mergevars(vtk_output,[2 3 4]);
vtk_output = mergevars(vtk_output,[3 4 5]);
vtk_output = mergevars(vtk_output,[4 5 6]);
vtk_output = mergevars(vtk_output,[5 6 7]);
vtk_output = mergevars(vtk_output,[6 7 8]);

% Adding Headings
vtk_output.Properties.VariableNames = {'ID','Position',...
    'Velocity','Force','Angular_Velocity','Torque'};
% vtk_output.Properties.VariableNames = {'ID','Type','Position',...
%     'Velocity','Force','Angular_Velocity','Torque'};
